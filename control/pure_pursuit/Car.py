...
class Car:
    ...
    def action(self, steer_angle, accel=0.0, dt=0.1):
        self.v += accel * dt
        ...
        self.yaw += self.v / Car.WHEEL_BASE * np.tan(steer_angle) * dt
        self.yaw = self.pi_2_pi(self.yaw)

        self.steer = np.clip(steer_angle, -Car.MAX_STEER, Car.MAX_STEER)
