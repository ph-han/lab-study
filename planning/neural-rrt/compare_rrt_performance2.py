...
def main():
    ...
    for ss in tqdm(STEP_SIZE, desc="Clearance Loop"):
        times_old = []
        times_new = []
        
        for i in tqdm(range(NUM_SEEDS), desc=f"SS {ss} Seeds", leave=False):
            seed = i  # 시드값 변경
    ...
    plt.xticks(x, STEP_SIZE)
    plt.grid(axis='y', linestyle=':', alpha=0.6)
    plt.legend(fontsize=12)
    
    output_filename = "comparison_performance_graph2.png"
