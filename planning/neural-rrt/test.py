    ...
    def neural_model(self, model, device, is_draw):
        ...
        if self.is_neural_mode:
            with torch.no_grad():
                ...
                output, recon = model(input_map, input_sc)
        ...
        if self.is_neural_mode:
            ...
            if is_draw:
                fig, axes = plt.subplots(1, 2, num=1, figsize=(15, 5), clear=True)
                fig.subplots_adjust(wspace=0.15, left=0.02, right=0.98, top=0.9, bottom=0.02)
                ...
                axes[0].imshow(reconst, cmap='gray', interpolation='bilinear')
                axes[0].set_title("Reconstruction", fontsize=10)
                axes[0].axis('off')

                axes[1].imshow(im_np, interpolation='nearest')
                alpha = out.copy()
                alpha[out < 1e-3] = 0.0
                alpha = np.clip(alpha * 2.0, 0.0, 0.8)

                im = axes[1].imshow(out, cmap='plasma', alpha=alpha, interpolation='bilinear')
                axes[1].set_title("Predicted sampling probability", fontsize=10)
                axes[1].axis('off')
                fig.colorbar(im, ax=axes[1], fraction=0.046, pad=0.04)
                fig.add_artist(plt.Line2D([0.5, 0.5], [0.08, 0.92], transform=fig.transFigure,
