...
def get_opt_path(paths, r_turn):
    opt_cost = calc_cost(paths[0][:], r_turn)
    opt_idx = 0
    num_of_possible_path = len(paths)

    for idx in range(1, num_of_possible_path):
        curr_cost = calc_cost(paths[idx][:], r_turn)
...
def gen_path(start_pos, dubins_params, r_turn, one_step=False, one_step_size=0):
    path_x = [start_pos[0]]
    path_y = [start_pos[1]]
    ...
    for p in dubins_params:
        if p[0] == 's':
            ...
            step = 1
            goal_angle = np.rad2deg(np.pi * 2 + p[1] if p[0] == 'l' and p[1] < 0 else p[1])
            base_x, base_y = path_x[-1], path_y[-1]
...
def dubins_path(curr_state, goal_state, r_turn):
    ...
    for idx, word in enumerate(dubins_words):
        ...
        if path is None:
            continue
        paths.append(path)
        # px, py, pyaw = gen_path(curr_state, path, r_turn)
...
if __name__ == "__main__":
    ...
    goal_state = [10, 16, np.deg2rad(90)]
    car = Car(*curr_state)
    ...
    paths = dubins_path(curr_state, goal_state, r_turn)
    opt_path, opt_cost = get_opt_path(paths, r_turn)
    # print("cost = ", opt_cost)

    px, py, pyaw = gen_path(curr_state, opt_path, r_turn)
    count = 0
